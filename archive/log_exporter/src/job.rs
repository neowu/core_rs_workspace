...
pub(crate) async fn process_log_job(state: Arc<AppState>, context: JobContext) -> Result<(), Exception> {
    ...
    upload_archive(today.checked_sub_days(Days::new(1)).expect("value must be valid"), state).await?;
    Ok(())
}
